...
- `Order Agent`는 주문 확인 뒤 카드 또는 현장결제 방식의 결제 시뮬레이션까지 이어서 처리한다.
...
- 입력 guardrail은 `pending_flow_state`와 직전 assistant 응답을 참고해 `확인`, `주문해줘`, `4명`, `그 메뉴`, `카드`, `현장결제`, `결제해줘` 같은 문맥형 follow-up을 허용한다.
- 레스토랑과 무관한 입력은 차단한다.
...
- `pending_flow_state`는 주문/예약 확인 단계뿐 아니라 주문 결제 단계도 함께 추적한다.
...
- 주문은 결제 시뮬레이션까지 끝난 뒤 완료로 간주하고, 주문과 예약이 완료되면 다음 턴 시작 agent는 다시 `Triage Agent`로 복귀한다.
